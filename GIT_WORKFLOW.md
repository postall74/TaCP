...
**Важно:** Все сообщения коммитов должны писаться на **русском языке** для удобства чтения командой.
...
❌ **Плохо (на английском, без контекста):**
```
Fix user position display issue in profile dropdown

Key features implemented:
- Fixed user position field display
- Updated store state management
```

✅ **Хорошо (на русском, по стандарту):**
```
fix(admin): Исправлено отображение должности в профиле пользователя

Проблема: В списке пользователей должность отображалась как прочерк (-), 
при смене должности значение не обновлялось в интерфейсе.

Решение: 
- Добавлено поле position в интерфейс User
- Обновлен экшен updateUser в authStore для обработки должности
- Исправлено модальное окно редактирования пользователя (UsersPage.tsx)
- Проверена сериализация объекта пользователя в localStorage
```
...
```
fix(ui): Устранены проблемы контрастности в светлой теме

Проблема: В светлой теме заголовки таблиц в разделе "Учёт времени" 
были нечитаемы (тёмный текст на тёмном фоне).

Решение:
- Заменены статические классы bg-dark/text-white на адаптивные токены
- Использованы design tokens: dark:bg-dark/bg-gray-100, dark:text-white/text-gray-900
- Протестировано переключение между светлой и тёмной темами
```
