...
Проверенная инструкция (Windows / PowerShell). Части: **1** требования, **2** фронтенд,
**3** бэкенд, **4** связка и авторизация, **5** онлайн-режим в локальной сети,
**6** повседневный цикл, **7** типовые проблемы (в конце).
Что дальше — дорожная карта в [DOCS.md § 15](./DOCS.md#15-дорожная-карта).  
Обновлённые требования к админ-панели, мастеру подбора и новым модулям (конфигуратор шкафов, тепловой расчёт, графика) — в [DOCS.md §§ 5–7](./DOCS.md#5-администрирование-и-аналитика).

**Полезные ссылки:**
- [TODOList.md](./TODOList.md) — список задач с планами работ по каждому пункту
- [GIT_WORKFLOW.md](./GIT_WORKFLOW.md) — регламент работы с Git (**коммиты на русском!**)
...
В логе ожидайте строку вида:

```
Фронтенд раздаётся из C:\dev\TaCP\dist — index.html от 14.05 18:30 (пересборка: npm run build)
```

**Дата в этой строке — ключ к диагностике**: если она раньше ваших последних
правок, значит `dist/` устарел (см. п. 5.6).
...
### 5.6. Порт 5085 показывает старую версию интерфейса

Порт **3000** (`npm run dev`) и порт **5085** (`dotnet run`) — это два разных
источника фронтенда:

| Порт | Что отдаёт | Свежесть |
|---|---|---|
| 3000 | vite dev-сервер — исходники напрямую | всегда актуальные (HMR) |
| 5085 | готовую сборку из `dist/` | **то, что собрано командой `npm run build`** |

Поэтому после любых изменений фронтенда на машине-сервере нужно пересобрать
`dist/` — иначе по сети будет видна старая версия:

```powershell
cd E:\dev\TaCP
npm run build        # разово — после правок
```

**Режим «всегда свежо» для разработки**: во втором терминале держите

```powershell
npx vite build --watch
```

— при каждом сохранении файла `dist/` пересобирается автоматически; участникам
достаточно обновить страницу в браузере (перезапускать `dotnet run` не нужно).

> Браузер больше не может «застрять» на старой версии: `index.html` отдаётся с
> заголовком `no-store` (запрашивается при каждом открытии), а хэшированные
> ассеты `/assets/*` при новом билде получают новые имена.
>
> Как убедиться, что сервер раздаёт свежее: в логе `dotnet run` строка
> «Фронтенд раздаётся из … index.html от <дата>» (см. п. 5.1).
...
| Порт 3000 показывает новое, а 5085 — старое | Порт 5085 отдаёт **сборку** `dist/`, а не исходники; после правок её не пересобрали | `npm run build` (или `npx vite build --watch` — автопересборка); дата сборки видна в логе API («index.html от …»). Подробнее — п. 5.6 |
...
| Чёрный экран при переходе на вкладку «Версии расчёта» | Рассинхрон контракта версий: сервер возвращает снимок вложенным (`snapshot:{cabinets,calc}`), а `VersionsTab` обращался к `v.cabinets.length`/`v.calc.total` → `TypeError` → краш React | Исправлено: `normalizeVersion()` в `store.ts` разворачивает `snapshot` в плоские поля, в `VersionsTab` — защитные `?.`/`?? 0`. Достаточно `npm run build` + обновить страницу; версии, созданные ранее, отображаются корректно |
