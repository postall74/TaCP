...
Тесты покрывают **чистую бизнес-логику** — то, что не зависит от UI и базы. Именно она
переносится между платформами, поэтому её стабильность критична.

**Frontend (Vitest):** `npx vitest run` (конфигурация — `vitest.config.ts`, отделена от
боевой `vite.config.js`).
- `src/utils/rules.test.ts` — движок совместимости: автомат↔шина (ошибка и «мощная шина
  снимает ошибку»), подсказка про гребёнку, УЗИП/ПЛК/панель/греющий кабель, пустой шкаф, сводка.
- `src/utils.test.ts` — расчётное ядро (наценка, трудозатраты по ролям, скидка→НДС, маржа,
  ТЗР и непредвиденные), разбор/экспорт CSV, `plural`, форматирование.

**Backend (xUnit):** `dotnet test backend/TkpApi.Tests`.
- `CatalogCsvTests` — чистый разбор прайсов: заголовок, битые строки, русские числа
  («1 800,50»), маппинг направлений, значения по умолчанию.
- `CalcEngineTests` — серверный расчётный движок; **сценарии зеркалят фронтовые**, чтобы
  две реализации не расходились.

Чтобы логика была тестируемой, её специально отделяют от инфраструктуры:
`CatalogCsv` расслоён на чистые `ParseLine/Parse` (без БД) и тонкий `Import` (запись в БД).

---

## 11. CI (GitHub Actions)

`.github/workflows/ci.yml` — на каждый push в `main` и на каждый PR:
- **frontend**: `npm ci` → `npx vitest run` → `npm run build`;
- **backend**: `dotnet test backend/TkpApi.Tests` → `dotnet build backend/TkpApi -c Release`.

Правило: PR сливается только с зелёным CI — это и есть автоматический «откат не понадобится».

---

## 12. Архитектура под коммерциализацию и мультиплатформенность

Проект с самого начала разложен так, чтобы его можно было адаптировать под других
заказчиков и собирать для Web / Desktop / Mobile без переписывания:

1. **Бизнес-логика — чистые модули, не зависящие от платформы.** Расчёт (`utils.ts:calcProject`
   ↔ `CalcEngine.cs`), правила (`rules.ts` ↔ будущий `RulesService.cs`), разбор прайсов
   (`CatalogCsv`). Они покрыты тестами и переезжают в любую платформу «как есть».
2. **Единые контракты данных.** `src/types.ts` и `backend/Models.cs` — зеркала одной модели
   (строковые Id, одинаковые поля). Клиент и сервер не могут «разойтись» незаметно.
3. **Конфигурация вместо кода.** Категории, типы шкафов, направления, ставки, реквизиты —
   данные (справочники и `settings`), а не хардкод. Под нового заказчика — новые справочники,
   не новая версия.
4. **Тонкие клиенты, общий сервер.** Web уже работает против C#-бэкенда (`src/api/client.ts`).
   Desktop (Avalonia/MAUI) и Mobile (MAUI) подключаются к **тем же эндпоинтам**, используя
   **тот же C#-ядро** (`Models`, `CalcEngine`, `CatalogCsv`) — максимум переиспользования.
   Web-фронтенд при желании упаковывается в Tauri/Electron без изменений.
5. **White-label.** Шапка документов, реквизиты, подписант/исполнитель — в настройках компании,
   поэтому «свой бренд» включается конфигурацией.

**Следующие шаги для коммерциализации** (см. дорожную карту): авторизация и роли, серверная
генерация PDF, история цен, согласование. Каждый из них опирается на уже выделенные модули.

---

## 13. Дорожная карта (что добавлять следующим)
