...
## 9. Дорожная карта (что добавлять следующим)

1. Авторизация и роли (инженер/менеджер), JWT в C#-бэкенде.
2. Проверка совместимости: автомат ↔ шина/клеммы по номинальному току (расширение `attrs`
   числовыми характеристиками, правила в отдельном модуле `rules.ts` / `RulesService.cs`).
3. Серверная генерация PDF (QuestPDF) с фирменным бланком.
4. История цен поставщиков, прайсы с датами действия.
5. Согласование ТКП: комментарии, подписи, уведомления.
